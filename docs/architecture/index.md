...
| [shared-helpers.md](shared-helpers.md) | You need `ResponseCapping`, `BridgeState`, `MiniJson`, `ProjectPaths`, `LogBuffer`, `LogWatchStore`, `SerializedValueExtractor`, `ActionToken`, `ActionScheduler`, (v2) `GameObjectResolver`, `ComponentSerializer`, `ComponentTypeResolver`, `MutationBody`, `MutationNodeBuilder`, `MutationAutoSave`, or (v2.5) `TransformParamReader`, `PrefabConnectionResolver`, and want their contracts without re-reading each source file. |
